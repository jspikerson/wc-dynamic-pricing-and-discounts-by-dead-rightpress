...
abstract class RightPress_Plugin_Settings extends RightPress_Settings implements RightPress_Plugin_Settings_Interface
{
    ...
    public function enqueue_assets()
    {
        ...
        // Enqueue Select2/SelectWoo scripts and styles (prefer WooCommerce-bundled SelectWoo)
        if (wp_script_is('selectWoo', 'registered')) {
            wp_enqueue_script('selectWoo');
        }
        else {
            wp_enqueue_script('rightpress-settings-select2-scripts', 'https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/js/select2.full.min.js', array('jquery'), '4.1.0-rc.0');
        }
        if (wp_style_is('select2', 'registered')) {
            wp_enqueue_style('select2');
        }
        else {
            wp_enqueue_style('rightpress-settings-select2-styles', 'https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/css/select2.min.css', array(), '4.1.0-rc.0');
        }
